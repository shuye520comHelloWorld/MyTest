...
namespace iParty.Services.Impl
{
    public sealed class QueryPartyService : Service
    {
        ...
        public object Get(QueryParty dto)
        {
            ...
            if (_context.User.IsDirector())
            {
                if (isFinished)
                {
                    ...
                    if (isDetail)
                    {
                        AppendFeedbackDetailTo(partyList, dto.PartyKey.Value);
                    }
                    else
                    {
            ...
            else if (_context.User.IsNotDirector() && !_context.User.IsVIP()) // Up from Level 15
            {
                var isDetail = dto.PartyKey.HasValue ? true : false;
                if (isFinished)
                {
        ...
        private void AppendSigninQueryTo(List<object> partyList)
        {
            ...
            {
                if (DateTime.Now.Date > signin.PartyDisplayEndDate)
                    continue;
                ...
                partyList.Add(new PartyrSigninQueryResult
                {
                    ...
                    PartyDisplayStartDate = signin.PartyDisplayStartDate,
                    PartyDisplayEndDate = signin.PartyDisplayEndDate,
        ...
        private void AppendFeedbackDetailTo(List<object> partyList, Guid partyKey)
        {
            var feedbacks = _partyQuery.GetFeedbackDetail(partyKey);
            foreach (var feedback in feedbacks)
            {
                partyList.Add(new PartyrFeedbackQueryResult
                {
                    EventKey = feedback.EventKey,
                    AppliedName = feedback.AppliedName,
                    AppliedContactId = feedback.AppliedContactId,
                    PartyKey = feedback.PartyKey,
                    Title = feedback.Title,
                    WorkshopName = feedback.WorkshopName,
                    WorkshopLocation = feedback.WorkshopLocation,
                    Category = feedback.Category,
                    Stage = PartyStage.FeedbackForInvitees,
                    PartyStartDate = feedback.PartyStartDate,
                    PartyEndDate = feedback.PartyEndDate,
                    PartyDisplayStartDate = feedback.PartyDisplayStartDate,
                    PartyDisplayEndDate = feedback.PartyDisplayEndDate,
                    PartyInviteesCount = feedback.PartyInviteesCount,
                    PartySigninCount = feedback.PartySigninCount,
                    IsSelfHost = feedback.IsSelfHost,
                    UnitInviteesCount = feedback.UnitInviteesCount,
                    SelfInviteesCount = feedback.SelfInviteesCount,
                    CoHostConsultants = feedback.CoHostConsultants,
                    UnitInvitationSummary = feedback.UnitInvitationSummary,
                    InvitationToken = feedback.InvitationToken,
                    SelfUnitID = feedback.SelfUnitID,
                    NewComersCount = feedback.NewComersCount,
                    PromotionCount = feedback.PromotionCount,
                    OrderedCustomerCount = feedback.OrderedCustomerCount
                });
            }
        }
        ...
        private void AppendInvitationQueryTo(List<object> partyList)
        {
            ...
            foreach (var invitation in invitations)
            {
                partyList.Add(new PartyrInvitationQueryResult
                {
                    ...
                    PartyDisplayStartDate = invitation.PartyDisplayStartDate,
                    PartyDisplayEndDate = invitation.PartyDisplayEndDate,
        ...
        private void AppendInvitationDetailTo(List<object> partyList, Guid partyKey)
        {
            ...
            foreach (var invitation in invitations)
            {
                partyList.Add(new PartyrInvitationQueryResult
                {
                    ...
                    PartySigninCount = invitation.PartySigninCount,
        ...
        private void AppendApplicationQueryTo(List<object> partyList)
        {
            ...
            foreach (var application in applications)
            {
                ...
                partyList.Add(new PartyCreatedQueryResult
                {
                    ...
                    PartyDisplayStartDate = application.PartyDisplayStartDate,
                    PartyDisplayEndDate = application.PartyDisplayEndDate
                });
            }
        }
        ...
        private void AppendHistoryApplicationQueryTo(List<object> partyList)
        {
            ...
            foreach (var application in applications)
            {
                ...
                partyList.Add(new PartyCreatedQueryResult
                {
                    ...
                    WorkshopLocation = application.WorkshopLocation,
                    Category = application.Category,
                    Stage = (!application.FeadbackEndDate.HasValue || DateTime.Now.Date > application.FeadbackEndDate ) 
                        ? PartyStage.Finished
                        : PartyStage.FeedbackForInvitees,
                    PartyStartDate = application.PartyStartDate,
                    PartyEndDate = application.PartyEndDate,
                    PartyDisplayStartDate = application.PartyDisplayStartDate,
                    PartyDisplayEndDate = application.PartyDisplayEndDate,
        ...
        private void AppendEventQueryTo(List<object> partyList)
        {
            ...
            {
                if (DateTime.Now < anEvent.EventStartDate)
                    continue;
        ...
        private void AppendEventDetailTo(List<object> partyList, Guid eventKey)
        {
            ...
            foreach (var anEvent in events)
            {
                ...
                partyList.Add(new ApplicationOpenedQueryResult
                {
                    ...
                    FeedbackEndDate = anEvent.FeedbackEndDate
                });
            }
        }
    }
