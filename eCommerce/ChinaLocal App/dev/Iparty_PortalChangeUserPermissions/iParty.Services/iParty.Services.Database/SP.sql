...
begin 
	select	p.UnitID,
			p.UniteeContactID as ContactID,
			p.FullName as UnitName,
			case 
				when p.UnitID =@UnitID then 1
				else 0
			end as IsMyUnit,
			COUNT(ip.PartyKey) AS [Count]
	from ( 
		select iu.PartyKey, iu.UnitId, iu.UniteeContactID, iu.FullName from iParty_Unitee iu with(nolock) where PartyKey = @PartyKey
		union
		select ipa.PartyKey, cu.UnitID, ipa.AppliedContactID as UniteeContactID, ipa.AppliedName as FullName 
		from iParty_PartyApplication ipa with(nolock) inner join
		btc_UnitMember cu with(nolock) on ipa.AppliedContactID = cu.Dir_ContactId
		where ipa.PartyKey = @PartyKey
	) p
	left join iParty_Invitation ip with(nolock) on p.UnitId = ip.OwnerUnitID and ip.PartyKey = @PartyKey
	where p.PartyKey = @PartyKey
	group by p.UnitID, p.UniteeContactID, p.FullName
end 
Go 
