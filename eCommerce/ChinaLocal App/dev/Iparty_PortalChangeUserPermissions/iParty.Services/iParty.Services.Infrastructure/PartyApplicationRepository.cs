...
namespace iParty.Services.Infrastructure
{
    public sealed class PartyApplicationRepository : IPartyApplicationRepository
    {
        ...
        {
            var applications = partyOperation.Join<PartyEntity, EventEntity>((p, e) => p.EventKey == e.EventKey
            ...
            var coHostApplicaitons = partyOperation.ExecEntityProcedure(GetCoHostApplications, new { contactId = _context.ConsultantContactId });
        ...
        {
            var applications = partyOperation.Join<PartyEntity, EventEntity>((p, e) => p.EventKey == e.EventKey
                && p.AppliedContactID == _context.ConsultantContactId && DateTime.Now.Date>p.EndDate);
        ...
        private ApplicationEntry BuildApplicationEntry(PartyEntity item, bool CanEdit)
        {
            ...
            application.InvitationCount = _invitationOperation.Count(x => x.PartyKey == item.PartyKey );
            application.CheckInCount = _invitationOperation.Count(x => x.PartyKey == item.PartyKey && (x.CheckInType != null||x.Status=="04") );
            return application;
        }

        public List<ApplicationEntryWithDetail> GetDetail(Guid PartyKey)
        {
            var applications = partyOperation.GetDataByFunc(t => t.AppliedContactID == _context.ConsultantContactId && t.PartyKey == PartyKey);
            var applicationlist = new List<ApplicationEntryWithDetail>();
            foreach (var item in applications)
            {
                var application = BuildApplicationEntryDetail(item, true);
            ...
            foreach (var item in coHostApplicaitons)
            {
                var application = BuildApplicationEntryDetail(item, false);
        ...
        public object Create(CreatePartyApplication dto)
        {
            ...
            foreach (var application in sameWorkShopApplications)
            {
                ...
                if (dto.EndDateUtc <= application.StartDate || dto.StartDateUtc >= application.EndDate)
                {
                    continue;
                }
            ...
            createItem.EndDate = dto.EndDateUtc;
            createItem.StartDate = dto.StartDateUtc;
            ...
            var coHostConsutlantIdList = new List<long>();
            foreach (var item in workShop.Consultants)
            {
                coHostConsutlantIdList.Add(item.ContactId);
            }
            ...
            if (dto.UnionConsultants != null && dto.UnionConsultants.Count >= 1)
            {
                ...
                {
                    if (!coHostConsutlantIdList.Contains(c))
                    {
            ...
            if (dto.UnionConsultants != null && dto.UnionConsultants.Count >= 1)
            {
                foreach (var c in dto.UnionConsultants)
                {
                    var dateString = string.Format("{0}年{1}月{2}日", createItem.StartDate.Year, createItem.StartDate.Month, createItem.StartDate.Day);
                    var strtype = "礼遇日";
        ...
        public object Update(UpdatePartyApplication dto)
        {
            ...
            validateApplicationDuration(dto.StartDateUtc, dto.EndDateUtc, eventDetail);
            party.StartDate = dto.StartDateUtc;
            party.EndDate = dto.EndDateUtc;
            ...
            foreach (var pc in preCoHostConsultants)
            {
                ...
                {
                    var dateString = string.Format("{0}年{1}月{2}日", party.StartDate.Year, party.StartDate.Month, party.StartDate.Day);
                    var strtype = "礼遇日";
        ...
        public object Delete(DeletePartyApplication dto)
        {
            ...
            if (party.OrganizationType == 1)
            {
                ...
                foreach (var pc in preCoHostConsultants)
                {
                    ...
                    var msg = string.Format(coHostConsultantDeleteMessageFormat, party.AppliedName, dateString, strtype);
        ...
        public List<InvitationEntry> GetPartyInvitation()
        {
            ...
            var invitations = partyOperation.Join<PartyEntity, EventEntity>(
                (p, e) => p.EventKey == e.EventKey &&
                    ...
                    p.StartDate > DateTime.Now.Date.AddDays(1));
