    ...
    public sealed class SqlHelper
    {
        ...
        public static int DataTableToSQLServer(string connectionString, string mappingTableName, DataTable dt)
        {
            using (SqlConnection destinationConnection = new SqlConnection(connectionString))
            {
                ...
                SqlTransaction st = destinationConnection.BeginTransaction();
        ...
        public static int DataTablesToSQLServer(string connectionString, Dictionary<string,DataTable> mappingTables)
        {
            using (SqlConnection destinationConnection = new SqlConnection(connectionString))
            {
                destinationConnection.Open();
                ...
                using (SqlBulkCopy bulkCopy = new SqlBulkCopy(destinationConnection))
                {
                    ...
                    try
                    {
                        ...
                        foreach (var m in mappingTables)
                        {
                            bulkCopy.DestinationTableName = m.Key;

                            foreach (DataColumn dc in m.Value.Columns)
                            {
                                bulkCopy.ColumnMappings.Add(dc.ColumnName, dc.ColumnName);
                            }
                            bulkCopy.WriteToServer(m.Value);
                        }
