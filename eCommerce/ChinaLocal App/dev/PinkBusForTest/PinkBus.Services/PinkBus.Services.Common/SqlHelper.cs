...
namespace PinkBus.Services.Common
{
    ...
    public sealed class SqlHelper
    {
        ...
        private static void AssignParameterValues(SqlParameter[] commandParameters, object[] parameterValues)
        {
            ...
            {
                commandParameters[0].Value = parameterValues[0];
            }
            else
            {
                ...
                {
                    commandParameters[i] = (SqlParameter)parameterValues[i];
                }
            }
        }
        ...
        private static SqlDataReader ExecuteReaderPage(SqlConnection connection, SqlTransaction transaction, CommandType commandType, string commandText, SqlParameter[] commandParameters, SqlConnectionOwnership connectionOwnership)
        {
            //create a command and prepare it for execution
            SqlCommand cmd = new SqlCommand();
            PrepareCommand(cmd, connection, transaction, commandType, commandText, commandParameters);
        ...
        public static SqlDataReader ExecuteReaderPage(string connectionString, CommandType commandType, string commandText, params SqlParameter[] commandParameters)
        {
            //create & open a SqlConnection
            SqlConnection cn = new SqlConnection(connectionString);
            cn.Open();
            ...
            {
                //call the private overload that takes an internally owned connection in place of the connection string
                return ExecuteReaderPage(cn, null, commandType, commandText, commandParameters, SqlConnectionOwnership.Internal);
            }
            catch
            {
                //if we fail to return the SqlDatReader, we need to close the connection ourselves
        ...
        public static SqlDataReader ExecuteReaderPage(string connectionString, string spName, params SqlParameter[] commandParameters)
        {
            //if we receive parameter values, we need to figure out where they go
            if ((commandParameters != null) && (commandParameters.Length > 0))
            {
                //call the overload that takes an array of SqlParameters
                return ExecuteReaderPage(connectionString, CommandType.StoredProcedure, spName, commandParameters);
            }
            //otherwise we can just call the SP without params
            else
            {
        ...
        #region DataTableToSQLServer

        /// <summary>
        /// DataTableToSQLServer
        /// </summary>
        /// <param name="connectionString">connectionString</param>
        /// <param name="mappingTableName">tableName</param>
        /// <param name="dt">data</param>
        /// <returns></returns>
        public static int DataTableToSQLServer(string connectionString, string mappingTableName, DataTable dt)
        {
            using (SqlConnection destinationConnection = new SqlConnection(connectionString))
            {
                ...
                using (SqlBulkCopy bulkCopy = new SqlBulkCopy(destinationConnection))
                {
                    try
                    {
                        bulkCopy.DestinationTableName = mappingTableName;
