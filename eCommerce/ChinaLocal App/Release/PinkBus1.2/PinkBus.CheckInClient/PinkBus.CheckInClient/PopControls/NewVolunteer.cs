    ...
    public partial class NewVolunteer : BaseForm
    {
        ...
        private void volunteerSave_btn_Click(object sender, EventArgs e)
        {
            ...
            if (volunteers.FindAll(c => c.DirectSellerId == SellerId_Text.Text).Count > 0)
            {
                ...
                //return;
                var volu = volunteers.FindAll(c => c.DirectSellerId == SellerId_Text.Text).FirstOrDefault();
                int msg = EventDAL.VolunteerCheckIn(volu);
                if(msg.Equals(2)){
                    MsgShow msgshow = new MsgShow("提示", "该志愿者今日已签到，请勿重复签到！");
                    msgshow.Owner = this;
                ...
                {
                    MsgShow msgshow = new MsgShow("提示", "该志愿者作废,不能签到！");
                    msgshow.Owner = this;
                    msgshow.StartPosition = FormStartPosition.CenterParent;
                    msgshow.ShowDialog();
                    if (msgshow.DialogResult.ToString().Equals("Cancel"))
                    ...
                    CheckInConfirm conf = new CheckInConfirm(volu);
                    conf.Owner = this;
                    conf.StartPosition = FormStartPosition.CenterParent;
                    conf.ShowDialog();
                    if (conf.DialogResult.ToString().Equals("Cancel"))
                    {
                        this.Close();
                    }
                }
            ...
            if (volunteers.FindAll(c => c.PhoneNumber == phone_text.Text).Count > 0)
            {
                ...
                return;
                //update by wang
                //var volu = volunteers.FindAll(c => c.PhoneNumber == phone_text.Text).FirstOrDefault();
                //CheckInConfirm conf = new CheckInConfirm(volu);
                //conf.Owner = this;
                //conf.StartPosition = FormStartPosition.CenterParent;
                //conf.ShowDialog();
