...
namespace PinkBus.CheckInClient.DAL
{
    public class EventDAL
    {
        ...
        //update by wang 插入志愿者签到数据
        public static int InsertVolunteerCheckIn(ConsultantTicket c) {
            int checkinfo = VolunteerCheckIn(c);
                ...
                using (SQLiteConnection conn = new SQLiteConnection(AppSetting.DataSource))
                {
                    using (SQLiteCommand cmd = new SQLiteCommand())
                    {
                        ...
                        try
                        {
                            #region  insert event
                            var dic = new Dictionary<string, object>();
                            dic["Key"] = Guid.NewGuid().ToString();
                            dic["EventKey"] = c.EventKey.ToString();
                            dic["MappingKey"] = c.MappingKey.ToString();
                            dic["CheckinDate"] = DateTime.Now.ToString();
                            dic["CheckinDay"] = DateTime.Now.Date.ToString();
                            dic["CreatedDate"] = DateTime.Now.ToString();
                            dic["CreatedBy"] = "ClientAdd";                               //不能为空
                            dic["UpdatedDate"] = DateTime.Now.ToString();
                            dic["UpdatedBy"] = "";
                            dic["ChangeStatus"] = 1;            //不能为空，默认为1
                            dic["SyncTimes"] = 0;                //可以为空
                            dic["SyncMsg"] = "";            //可以为空
                            sh.Insert("PB_VolunteerCheckin", dic);
                            #endregion
        ...
        //update by wang 查询志愿者当天是否签到
        public static int VolunteerCheckIn(ConsultantTicket con)
        {
            int msg = 0;          //可以签到
            string sqlS = "select Checkinday from PB_VolunteerCheckin where MappingKey='" + con.MappingKey + "' and EventKey='" + con.EventKey + "'";
            ...
            if (vol.FindAll(e => e.CheckinDay == DateTime.Now.Date).Count > 0)
            {
                msg = 2;           //"该志愿者今日已签到，请勿重复签到！"
                return msg;
            }
            return msg;
        }
        ...
        //update by wang
        public static List<Ticket> QueryTickets(string customerkey)
        {
            string sql = "select * from PB_Ticket where CustomerKey='" + customerkey.ToString().ToLower() + "'";
            List<Ticket> ticktes = new List<Ticket>();
            OperationSqlite(e => ticktes = e.Selects<Ticket>(sql));
            return ticktes;
        }
        //更新签到状态 update by wang
        public static int UpdateTicket(Ticket ticket)
        {
            if (ticket.TicketStatus.Equals(5))
            {
                return 2;
            }
            else if (ticket.TicketStatus.Equals(4))
            {
                return 1;
            }
            else {
                using (SQLiteConnection conn = new SQLiteConnection(AppSetting.DataSource))
                {
                    using (SQLiteCommand cmd = new SQLiteCommand())
                    {
                        ...
                        try
                        {
                            string sql = "update PB_Ticket set TicketStatus =5,ChangeStatus=1,updatedby='SMS', CheckinDate='" + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "' where TicketKey ='" + ticket.TicketKey.ToString().ToLower() + "'";
        ...
        public static bool PassCodeCheckin(Guid eventKey, string code, ref Guid customerKey, ref string customerName, ref TicketType ticketType, out string err)              //update by wang
        {
