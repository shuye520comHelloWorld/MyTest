...
namespace PinkBus.CheckInClient.Helper
{
    ...
    public class SQLiteHelper
    {
        ...
        public List<T> Selects<T>(string sql)
        {
            DataTable dt = Select(sql, new List<SQLiteParameter>());
        ...
        {
            DataTable dt = Select(sql, new List<SQLiteParameter>());
            if (dt.Rows.Count < 1) return Activator.CreateInstance<T>();


            List<T> LT = JsonConvert.DeserializeObject<List<T>>(JsonConvert.SerializeObject(dt));
            return LT[0];
        }
