...
namespace PinkBus.CheckInClient.Helper
{
    public class NPOIHelper
    {
        ...
        public static void ExportAllCheckin(string savedFileName, List<CustomerTicket> data,Event ev)
        {
            var tmp = AppDomain.CurrentDomain.BaseDirectory + @"\Data\Checkins.xlsx";
            using (FileStream file = new FileStream(tmp, FileMode.Open, FileAccess.Read))
            {
                var workBook = new XSSFWorkbook(file);
                var sheet = workBook.GetSheet("Sheet1");
                ...
                foreach (var item in data)
                {
                    logger.Debug(item.UserName.ToString());
                    var row = sheet.CreateRow(i++);

                    row.CreateCell(0).SetCellValue(item.UserType.ToString());
                    row.CreateCell(1).SetCellValue(item.UserName.ToString());
                    row.CreateCell(2).SetCellValue(item.UserPhoneNumber);
                    row.CreateCell(3).SetCellValue(item.CheckinStatus);

                    foreach (ICell c in row.Cells)
                    {
                        c.CellStyle = cStyle;
                    }
                   // row.GetCell(0).CellStyle =row.GetCell(1).CellStyle= row.GetCell(2).CellStyle=row.GetCell(3).CellStyle =cStyle;
                    //SetRowStyle(row, workBook, 4);
                }
